...

{
			...
			const graphlab::empty& empty) 
	{
        ...
        if(vertex.data().sent == true){
			...
			// dont do anything
            COUT<<"  cat: "<<vertex.id() <<ENDL;
			std::map<graphlab::vertex_id_type, Triple> temp;
								
			for(unsigned int i=0; i < temp1.size(); i++)
			{
                COUT << "  cat | temp1[i].dest_art" << temp1[i].dest_art << ENDL;
			...
			edge_type& edge) const 
	{
        ...
        std::cout<< "scatter: from " << vertex.id() << "->" << other.id() << std::endl;
		if(other.data().type==14 && vertex.data().type == 14) //category to category
        {
			...
			for(int i=0; i < 1/*temp1.size()*/; i++) // runs once?
			{
				//if(temp1[i].from_last_art < 12 && temp1[i].last_cat !=other.id() )
				{
					...
					if(other.data().seen.find(msg.dest_art)
                       != other.data().seen.end())
					{
						msg.from_src += 1;
						msg.from_last_art += 1;
		 	...
		 	if(for_cat.collection.size() !=0){
   			...
   			our_msg for_art;
   			std::cout<<"  cat:" << vertex.id() << " art: "<< other.id() << std::endl;
			for(unsigned int i=0; i<temp1.size(); i++){
				//If it exists in the add_neighbours
                // print out the dest art
                std::cout<<"  cat, dest art:" << temp1[i].dest_art << std::endl;
				...
				{
                    std::cout << "  cat->neighbor: " <<  other.id() << std::endl;
					Quad tp2(temp1[i].dest_art,
                             temp1[i].from_src+1,
                             temp1[i].from_last_art+1 );
		...
		{
            std::cout<< "art-cat (v): " << vertex.id() << std::endl;
			our_msg for_cat1;
