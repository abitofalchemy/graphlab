	...
	public graphlab::IS_POD_TYPE
{
	...
	return graphlab::OUT_EDGES; // 08Aug15/SA: changed this to OUT
	}
					 ...
					 edge_type& edge) const
	{
		...
		vertex_id_type otherid = edge.target().id(); //edge.source().id() == vertex.id() ?
							     //	 edge.target().id() : edge.source().id();
		//	//COUT << vertex.id() << " "<< edge.source().id() << " "<< edge.target().id()	<< ENDL;
		//	//COUT << edge.target().id(). << ENDL;

		if ((vertex.data().type== 0 && edge.target().data().type == 0) ||
			(vertex.data().type== 0 && edge.target().data().type == 14))
		{
					...
					our_msg>
{
			 				...
			 				other.data().isDead == false) //category to article ***************
	{
		...
		for(unsigned int i=0; i<vrtx_dists.size(); i++){
			...
			{
				//COUT	<< "	cat->nei: " <<vrtx_dists[i].dest_art 
				//			<< "->" << other.id() << ENDL;
...
int main( int argc, char** argv) {
	...
	graph_type graph(dc, clopts);
        graph.load("/data/saguinag/datasets/enwiki/page.txt" ,
				 all_vertex_parser);
        //graph.load("/data/saguinag/datasets/enwiki/top_ranked_bs_pagelinks.tsv",
	graph.load("/home/saguinag/CategoryPaths/wiki_data/wpg_paths/adjlist/8083_act_neis.tsv",
				 line_parser_art);
	//graph.load("/data/saguinag/datasets/enwiki/top_ranked_cate_links.tsv",
        graph.load("/data/saguinag/datasets/enwiki/categorylinks.txt",
				 line_parser_categ);
	logstream(LOG_INFO) << "Finished loading all graphs.\n";
